...
namespace AsianMarketplace_WebAPI.Tests.Controllers
{
    public class CartItemControllerTests
    {
        ...
        public async Task GetCartItems_ShouldReturnAllCartItems()
        {
            ...
            //_mockDbContext.Setup(db => db.CartItems.ToList()).Returns(cartItems);

            // Setup the IQueryable property of DbSet<CartItem>
            //_mockCartItems.As<IQueryable<CartItem>>().Setup(m => m.Provider).Returns(cartItems.Provider);
            //_mockCartItems.As<IQueryable<CartItem>>().Setup(m => m.Expression).Returns(cartItems.Expression);
            //_mockCartItems.As<IQueryable<CartItem>>().Setup(m => m.ElementType).Returns(cartItems.ElementType);
            //_mockCartItems.As<IQueryable<CartItem>>().Setup(m => m.GetEnumerator()).Returns(cartItems.GetEnumerator());

            // Setup the mock DbSet<CartItem> to support async operations
            //_mockCartItems.Setup(m => m.ToListAsync(default)).ReturnsAsync(cartItems.ToList());

            //_mockDbContext.Setup(db => db.CartItems).Returns(_mockCartItems.Object);
            ...
            // Verify whether the result is of type OkObjectResult and store it
            var okResult = Assert.IsType<ObjectResult>(result.Result);
            // Verify whether the value of that result above is of type List<CartItemDTO> and store it
            var returnValue = Assert.IsType<List<CartItemDTO>>(result.Value);
